...
from fastapi_learning.crud import GenshinRoleCRUD
...
async def create_genshin_role(
    role: Annotated[GenshinRoleCreate, Body(description="角色信息")], db: Annotated[AsyncSession, Depends(get_db)]
):
...
async def delete_genshin_role(
    role_id: Annotated[uuid.UUID, Path(description="角色 UUID")], db: Annotated[AsyncSession, Depends(get_db)]
):
...
async def get_all_roles(db: Annotated[AsyncSession, Depends(get_db)]):
