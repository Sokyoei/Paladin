...
ctypes_learning = np.ctypeslib.load_library("ctypes_learning", Path(".").absolute())
...
ctypes_learning.array_add.restype = c_int_3x3_p
aa = np.ctypeslib.as_ctypes(arg)
ret = ctypes_learning.array_add(aa)
...
stus = (Student * 3)((1, b"Ahri"), (2, b"Sokyoei"), (3, b"Nono"))
# AhriC.student_add.argtypes = [POINTER(Student)]
ctypes_learning.student_add.restype = POINTER(Student)
stus_ret = ctypes_learning.student_add(stus)
...
class Ahri(Structure):
    _fields_: ClassVar[list[tuple]] = [("age", c_int), ("score", POINTER(c_int)), ("score_len", c_int)]
...
ctypes_learning.print_Ahri(ahris, 3)
ctypes_learning.update_Ahri(ahris, 3)
...
# AhriC.SOKYOEI = 100
ctypes_learning.set_SOKYOEI(100)
ctypes_learning.print_SOKYOEI()
